...
简介: 一个浏览器中的大模型上下文演化沙盒，用于观察可编辑的 context genome 如何在资源、竞争、突变和选择压力下塑造代理行为。
...
系统流程、prompt 角色和上下文继承机制见 [Architecture](docs/architecture.md)。

## 安装与启动

需要 Python 3.11 或更新版本。浏览器观察器只依赖 Python 标准库，直接从项目根目录启动即可：
...
```bash
make run      # 启动本地观察器
make doctor   # 检查 Python、Node、服务健康、端口、run.sh 和 LLM 运行时提示
make test     # 运行单元测试
...
健康响应会报告 product/version、tick、preset、agent mode、有机体数量、token budget 状态，以及 LLM runtime 是否已配置。它不会返回 API key。
...
dev container 会安装 editable package，自动运行 `./run.sh --host 0.0.0.0 --port 8765`，并让 Codespaces 转发这个端口为 `Context Genome`。浏览器预览打开后，在 `Tune -> LLM Runtime` 里填写模型 Base URL 和 API key；key 只保存在服务端内存中，不会提交到仓库。
...
想快速理解项目，可以先看 [Demo Gallery](docs/demo-gallery.md)。里面有三组确定性的零 token 演示：

| 案例 | 展示内容 | 复现设置 |
| --- | --- | --- |
| 稳定采集扩张 | 中等资源环境如何选择紧凑的采集、修复、复制行为 | `sandbox`, seed `30` |
| 灾害压力选择最小上下文 | 突变和灾害如何偏好维护成本更低的小上下文 | `wild`, seed `16` |
| 冲突压力下的选择 | 固定竞争开局中，谱系如何在拥挤和冲突下被选择 | `tournament`, seed `19` |
...
2. **选择实验模板**
   在 `Tune` 页使用 `Quick Smoke`、`Low-Cost LLM`、`Selection Pressure` 或 `Cache Study` 填入推荐参数。模板只调整控件；需要点击 `Reset` 才会重建初始世界。

3. **设置初始世界**
   在 `Tune` 页可以调节网格宽高、初始格点能量、矿物、辐射、初始种群数量、初始有机体能量和单格容量。
   这些参数会在下一次 `Reset` 时生效。`Stop Conditions` 可以让自动运行在全灭、tick 上限、运行时间上限、长时间无变化或单一谱系占优时暂停。
...
8. **编辑有机体上下文**
   ...
   这相当于研究者干预一个生命体的方法论、能力权重或自我叙事。`LLM Inspector` 会显示这个有机体最近一次 prompt messages、模型原始返回、解析后的 action、token 用量和 cache 用量。
...
11. **比较结果**
    `Results` 页会汇总当前世界，并对比已导出的实验，包括种群、谱系、出生、死亡、token、cache hit、完整性和领先谱系。
...
- `Tune`: 实验模板、世界参数、自动停止条件、选择压力、LLM 运行时、导出和加载。
- `Results`: 当前结果摘要和已导出实验对比。
- `Cell`: 当前格点和格点内有机体。
- `Edit`: 种子上下文、单个有机体上下文编辑和 LLM Inspector。
...
docs/          架构说明、demo gallery、截图和脱敏示例
...
- **不确定本地哪里出问题**：运行 `make doctor`。如果服务已启动，它会读取 `/api/health`；否则会检查目标端口是否空闲。
...
本地开发流程和 PR 检查见 [CONTRIBUTING.md](CONTRIBUTING.md)。API key、导出 run、截图等安全注意事项见 [SECURITY.md](SECURITY.md)。
